...
const ACHIEVEMENTS: Achievement[] = [
  ...
  { key: 'solve_200', title: 'Double Century', description: 'Solve 200 problems', icon: <Rocket className="h-5 w-5" />, condition: s => s.totalSolved >= 200, category: 'solving' },
  // Mastery
  ...
  { key: 'easy_25', title: 'Easy Street', description: 'Solve 25 easy problems', icon: <Shield className="h-5 w-5" />, condition: s => s.easyCount >= 25, category: 'mastery' },
  ...
  { key: 'medium_25', title: 'Medium Well', description: 'Solve 25 medium problems', icon: <Gem className="h-5 w-5" />, condition: s => s.mediumCount >= 25, category: 'mastery' },
  ...
  { key: 'hard_15', title: 'Hardcore', description: 'Solve 15 hard problems', icon: <Award className="h-5 w-5" />, condition: s => s.hardCount >= 15, category: 'mastery' },
  { key: 'hard_30', title: 'Legendary', description: 'Solve 30 hard problems', icon: <Crown className="h-5 w-5" />, condition: s => s.hardCount >= 30, category: 'mastery' },
  // Streak
  ...
  { key: 'streak_14', title: 'Fortnight Fighter', description: '14-day practice streak', icon: <Flame className="h-5 w-5" />, condition: s => s.currentStreak >= 14, category: 'streak' },
  { key: 'streak_30', title: 'Monthly Master', description: '30-day practice streak', icon: <Flame className="h-5 w-5" />, condition: s => s.currentStreak >= 30, category: 'streak' },
  // Special
  ...
  { key: 'interview_1', title: 'Interview Ready', description: 'Complete a mock interview', icon: <Brain className="h-5 w-5" />, condition: s => s.interviewsDone >= 1, category: 'special' },
  { key: 'interview_5', title: 'Interview Pro', description: 'Complete 5 mock interviews', icon: <Brain className="h-5 w-5" />, condition: s => s.interviewsDone >= 5, category: 'special' },
  { key: 'creator', title: 'Problem Creator', description: 'Create a custom problem', icon: <BookOpen className="h-5 w-5" />, condition: s => s.customProblems >= 1, category: 'special' },
  { key: 'sharer', title: 'Community Hero', description: 'Share a solution', icon: <Heart className="h-5 w-5" />, condition: s => s.sharedSolutions >= 1, category: 'special' },
];
        ...
        <Card className="border-primary/10">
          <CardContent className="pt-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-foreground">You have unlocked {unlockedKeys.size} out of {ACHIEVEMENTS.length} badges.</p>
              </div>
              <div className="text-right space-y-0.5">
                <p className="text-sm text-muted-foreground flex items-center justify-end gap-1"><Flame className="h-3 w-3 text-orange-500" /> {stats?.currentStreak || 0} day streak</p>
