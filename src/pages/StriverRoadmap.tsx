...
import {
  ArrowLeft, ChevronDown, ChevronRight, CheckCircle2, Circle, Clock,
  Bookmark, BookmarkCheck, Loader2, Map, Target, TrendingUp, Sparkles, RotateCcw,
} from 'lucide-react';
...
import { STRIVER_ROADMAP, getTotalProblems, getDifficultyBg, type RoadmapProblem } from '@/lib/striver-roadmap-data';
...
const StriverRoadmap = () => {
  ...
  const stats = useMemo(() => {
    ...
    const easy = STRIVER_ROADMAP.flatMap(t => t.problems).filter(p => p.difficulty === 'Easy' && progress[p.key]?.solved).length;
    const medium = STRIVER_ROADMAP.flatMap(t => t.problems).filter(p => p.difficulty === 'Medium' && progress[p.key]?.solved).length;
    const hard = STRIVER_ROADMAP.flatMap(t => t.problems).filter(p => p.difficulty === 'Hard' && progress[p.key]?.solved).length;
    return { total, solved, revision, easy, medium, hard };
  }, [progress]);

  const recommendation = useMemo(() => {
    // Find weakest topic and suggest next unsolved problem
    ...
    <div className="flex h-screen flex-col bg-background">
      ...
      <div className="flex items-center gap-3 border-b border-panel-border bg-ide-toolbar px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold">Striver SDE Sheet Roadmap</span>
        </div>
        ...
        <div className="w-72 shrink-0 border-r border-panel-border bg-ide-sidebar overflow-auto">
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {STRIVER_ROADMAP.map(topic => {
                const solved = topic.problems.filter(p => progress[p.key]?.solved).length;
                const pct = topic.problems.length > 0 ? (solved / topic.problems.length) * 100 : 0;
                return (
                  <button
                    key={topic.name}
                    onClick={() => {
